...
function Panel2(props) {
  ...
  useEffect(() => {
    if (marketRQ.data) {
      setState((prev) => ({
        ...
        price: {
          ...
          footer: (
            marketData?.current_price.usd * (metricsData?.supply ?  metricsData?.supply?.total : 1)
          ).toFixed(2),
        },
      }));
    }
    ...
    if (miningRQ.data) {
      // TODO: const miningData = miningRQ.data.data.result;
  ...
  return (
    <section className={styles.panelTwoContainer}>
      ...
      <DetailCard
        ...
        rewardLabel="Total"
        footerLabel="Fees "
        footerValue={`${intlNum(state.stake?.footer)} fxc`}
        delayTime={`${cardRefreshTimeout}s`}
      />
      ...
      <DetailCard
        ...
        reserve={`${state.prime?.reserve} fxc`}
        rewardLabel="Reward"
        reward={`${state.prime?.reward} fxc`}
        footerLabel="Fees"
        footerValue={`${intlNum(state.prime?.footer)} fxc`}
        delayTime={`${cardRefreshTimeout}s`}
      />
      <DetailCard
        type="basic"
        ...
        unit="H/s"
        reserveLabel="Reserve"
        reserve={`${intlNum(state.hash?.reserve)} fxc`}
        rewardLabel="Reward"
        reward={`${state.hash?.reward} fxc`}
        footerLabel="Fees"
        footerValue={`${intlNum(state.hash?.footer)} fxc`}
        delayTime={`${cardRefreshTimeout}s`}
      />
    </section>
  );
